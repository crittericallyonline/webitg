...
void DialogDriver_HTML::OK( CString sMessage, CString sID )
{
	// g_bAllowHush = sID != "";
	// g_sMessage = sMessage;
	// AppInstance handle;
...
void DialogDriver_HTML::Error( CString sError, CString sID )
{
	g_sMessage = sError;
}

Dialog::Result DialogDriver_HTML::AbortRetryIgnore( CString sMessage, CString ID )
{
	// EM_ASM({
	// 	console.log()
	// }, sMessage, ID);
...
Dialog::Result DialogDriver_HTML::AbortRetry( CString sMessage, CString sID )
{
	// CString sWindowTitle = WINDOW_TITLE.IsLoaded() ? WINDOW_TITLE.GetValue() : "";
